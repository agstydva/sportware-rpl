...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['updateProfilePhoto'])) {
        ...
        if($check !== false) {
            ...
            $uploaddir = $_SERVER['DOCUMENT_ROOT'].'/sportware/img/';
            $uploadfile = $uploaddir . $newfilename;

            if (move_uploaded_file($_FILES['userimage']['tmp_name'], $uploadfile)) {
                echo "<script>alert('Profile photo updated successfully!');
                        // Force page reload to refresh cache
                        window.location.href = window.location.href.split('?')[0] + '?page=userManage&updated=' + new Date().getTime();
                    </script>";
            } else {
    ...
    if(isset($_POST['removeProfilePhoto'])) {
        $id = $_POST["userId"];
        $filename = $_SERVER['DOCUMENT_ROOT']."/sportware/img/person-".$id.".jpg";
        if (file_exists($filename)) {
            unlink($filename);
            echo "<script>alert('Profile photo removed successfully!');
                    // Force page reload to refresh cache
                    window.location.href = window.location.href.split('?')[0] + '?page=userManage&removed=' + new Date().getTime();
                </script>";
        }
        else {
            echo "<script>alert('No profile photo found to remove.');
                window.location=document.referrer;
            </script>";
        }
    }
