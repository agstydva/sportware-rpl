...
/* Profile image styling untuk admin */
.admin-profile-image {
...
<div class="container-fluid" style="margin-top:98px">
	...
	<div class="row">
        <div class="col-lg-12">
            <button class="btn btn-success float-right btn-sm" data-toggle="modal" data-target="#newUser"><i class="fa fa-plus"></i> New user</button>
        </div>
	</div>
	    <br>
	<div class="row">
                    ...
                    <tbody>
                        <?php
                            ...
                            while($row=mysqli_fetch_assoc($result)) {
                                ...
                                // Use database timestamp for more accurate cache busting
                                $profileImagePath = $_SERVER['DOCUMENT_ROOT'] . '/sportware/img/person-' . $Id . '.jpg';
                                $fileTime = file_exists($profileImagePath) ? filemtime($profileImagePath) : 0;
                                $cacheBuster = '?t=' . max($profilePicUpdated, $fileTime, time());
                                $imageSrc = '/sportware/img/person-' . $Id . '.jpg' . $cacheBuster;

                                echo '<tr>
                                    <td>' .$Id. '</td>
                                    <td><img src="' . $imageSrc . '" alt="image for this user" onError="this.src =\'/sportware/img/profilePic.jpg\'" class="admin-profile-image" width="100" height="100"></td>
                                    <td>' .$username. '</td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <div class="row mx-auto" style="width:112px">
                                            <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#editUser' .$Id. '" type="button">Edit</button>';
                                            if($Id == 1) {
        ...
        <div class="modal-body">
            ...
            <div class="text-left my-2 row" style="border-bottom: 2px solid #dee2e6;">
                ...
                <div class="form-group col-md-4">
                    <?php 
                    $modalProfileImagePath = $_SERVER['DOCUMENT_ROOT'] . '/sportware/img/person-' . $Id . '.jpg';
                    $modalCacheBuster = file_exists($modalProfileImagePath) ? '?t=' . filemtime($modalProfileImagePath) : '';
                    $modalImageSrc = '/sportware/img/person-' . $Id . '.jpg' . $modalCacheBuster;
                    ?>
                    <img src="<?php echo $modalImageSrc; ?>" alt="Profile Photo" width="100" height="100" onError="this.src ='/sportware/img/profilePic.jpg'">
                    <form action="partials/_userManage.php" method="post">
                        <input type="hidden" id="userId" name="userId" value="<?php echo $Id; ?>">
            ...
            <form action="partials/_userManage.php" method="post">
                ...
                <div class="form-group row my-0">
                    <div class="form-group col-md-6 my-0">
                        <b><label for="phone">No Telp:</label></b>
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text" id="basic-addon">+62</span>
                            </div>
...
$(document).ready(function() {
    // Check if page was reloaded after image update
    var urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('updated') || urlParams.has('removed')) {
        // Force refresh all profile images with new timestamp
        ...
        // Clean up URL
        var cleanUrl = window.location.href.split('?')[0] + '?page=userManage';
        window.history.replaceState({}, document.title, cleanUrl);
    }
    
    // Check localStorage for profile images removed by users
    var timestamp = new Date().getTime();
    var keysToRemove = [];
    
    for (var i = 0; i < localStorage.length; i++) {
        var key = localStorage.key(i);
        if (key && key.startsWith('profileImageRemoved_')) {
            var userId = key.replace('profileImageRemoved_', '');
            var removeTime = localStorage.getItem(key);
            
            // If removal was within last 5 minutes, refresh this user's image
            if (timestamp - removeTime < 300000) { // 5 minutes
                $('img[src*="person-' + userId + '"]').each(function() {
                    var currentSrc = $(this).attr('src');
                    var newSrc = 'img/profilePic.jpg?t=' + timestamp;
                    $(this).attr('src', newSrc);
                    $(this).attr('onerror', "this.src='/sportware/img/profilePic.jpg'");
                });
            }
            
            // Clean up old entries (older than 5 minutes)
            if (timestamp - removeTime > 300000) {
    ...
    // Handle form submissions for profile photo management
    $('form[action="partials/_userManage.php"]').on('submit', function(e) {
        ...
        // Reset after timeout (form will actually submit)
        setTimeout(function() {
    ...
    // Auto-refresh untuk update foto profile
    setInterval(function() {
        // Check if there's a signal that profile images were updated
        if(localStorage.getItem('profileImageUpdated') === 'true') {
            localStorage.removeItem('profileImageUpdated');
            
            // Refresh all profile images dengan timestamp baru
            $('img[src*="person-"]').each(function() {
