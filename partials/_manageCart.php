...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['removeItem'])) {
        ...
        if($result) {
            ...
            echo "<script>alert('Error removing item');
                    window.location.href = document.referrer;
                </script>";
        }
    }
    if(isset($_POST['removeAllItem'])) {
        ...
        $result = mysqli_query($conn, $sql);
        if($result) {
            echo "<script>alert('All items removed successfully');
                    window.location.href = document.referrer;
                </script>";
        } else {
            echo "<script>alert('Error removing all items');
                    window.location.href = document.referrer;
                </script>";
        }
    }
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                echo '<script>alert("Thanks for ordering with us. Your order id is ' .$orderId. '.");
                    window.location.href="/sportware/index.php";  
                    </script>';
                    exit();
            }
        } 
